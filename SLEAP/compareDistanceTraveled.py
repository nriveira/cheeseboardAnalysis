...
def plot_distance_traveled_all(pre, interfere, post, color, offset, preIntOffset, postIntOffset, label, scatter=True):
    ...
    plt.errorbar(interfereMean.index+10+offset+preIntOffset, interfereMean.values, yerr=interfereSem.values, color=color)
    plt.errorbar(postMean.index+20+offset+preIntOffset+postIntOffset, postMean.values, yerr=postSem.values, color=color)
    if scatter:
        plt.scatter(preMatrix.index.repeat(preMatrix.shape[1])+offset, preMatrix.values.flatten(), color=color, alpha=0.3)
        plt.scatter(interfereMatrix.index.repeat(interfereMatrix.shape[1])+10+offset+preIntOffset, interfereMatrix.values.flatten(), color=color, alpha=0.3)
        plt.scatter(postMatrix.index.repeat(postMatrix.shape[1])+20+offset+preIntOffset+postIntOffset, postMatrix.values.flatten(), color=color, alpha=0.3)
